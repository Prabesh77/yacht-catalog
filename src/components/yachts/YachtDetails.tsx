...
export default function YachtDetails({ yacht }: YachtDetailsProps) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-gray-100/50 animate-fade-in-up">
      <div className="relative h-96 md:h-[500px] w-full overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-8 md:p-12 animate-fade-in" style={{ animationDelay: '0.3s', opacity: 0 }}>
          <div className="mb-3">
      ...
      <div className="p-6 md:p-10 bg-gradient-to-b from-white via-cream/30 to-white">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          {[
            { icon: 'length', label: 'Length', value: `${yacht.length}m` },
            { icon: 'guests', label: 'Guests', value: yacht.maxPassengers },
            { icon: 'bedrooms', label: 'Bedrooms', value: yacht.bedrooms },
            { icon: 'crew', label: 'Crew', value: yacht.maxCrew },
          ].map((spec, index) => (
            <div
              key={spec.icon}
              className="flex items-center gap-3 p-4 bg-gradient-to-br from-gold/10 to-gold/5 rounded-xl border border-gold/20 animate-fade-in-up"
            ...
            >
              ...
              <div>
                <p className="text-xs text-gray-500 font-medium uppercase tracking-wide mb-1">{spec.label}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-10">
          <div className="lg:col-span-2">
            <div className="mb-8 animate-fade-in-up" style={{ animationDelay: '0.8s', opacity: 0 }}>
              ...
              <div className="animate-fade-in-up" style={{ animationDelay: '0.9s', opacity: 0 }}>
                ...
                <div className="flex flex-wrap gap-3">
                  {toysList.map((toy, index) => (
                    <span
                      key={index}
                      className="px-4 py-2 bg-gradient-to-br from-gold/10 to-gold/5 border border-gold/20 rounded-lg text-sm font-medium text-primary uppercase tracking-wide animate-fade-in"
                      style={{ animationDelay: `${1.0 + index * 0.05}s`, opacity: 0 }}
                    >
                      {toy}
                    </span>
                  ))}
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-8 p-6 bg-gradient-to-br from-primary via-secondary to-primary rounded-2xl shadow-xl border border-gold/20 animate-fade-in-up" style={{ animationDelay: '0.85s', opacity: 0 }}>
